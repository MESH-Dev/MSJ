jQuery(document).ready(function($){
  ...
  function resizeScrollCont(){
    ...
    $('body.home #scrollCont').height($('#scrollJack > *:eq('+curDiv+')').height());
  }
  resizeScrollCont();
  $(window).resize(resizeScrollCont);

  function nextStory(nextID){
    var nextID = parseInt(nextID);
    $('#scrollCont').attr('data-div',nextID+1);
    resizeScrollCont();
    var curLoc = $('.home-story').eq(nextID);
    var curPos = curLoc.position();
    $('#scrollJack').scrollTo(curLoc);
  }
